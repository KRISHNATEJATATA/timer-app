  ...
  const api = {
    ...
    os: {
      ...
      async showFolderDialog(title, defaultPath) {
        ...
        if (options.chosenFolder !== undefined) return options.chosenFolder;
        return api.chosen !== undefined ? api.chosen : null;
      },
...
(async function run() {
  ...
  // The block is scoped to the unreadable file only: moving to a healthy
  // folder must work without a restart.
  quarantineFailFake.chosen = NEW_DIR;
  await quarantineFailApp.openSettings();
  await quarantineFailApp.browseForDataDir();
  await quarantineFailApp.applySettings();
  await settle();
  const rescueWrite = logWrites(quarantineFailFake).find((w) => w.path === NEW_DIR + '/log.json');
  assert.ok(rescueWrite, 'writes allowed to a new folder despite the blocked old file');
  assert.strictEqual(rescueWrite.data.sessions.length, 0, 'fresh start at the new folder');
  assert.strictEqual(
    quarantineFailFake.filesystem._exists[DEFAULT_DIR + '/log.json'],
    '{ not valid json !!',
    'blocked file still untouched after the escape hatch'
  );
